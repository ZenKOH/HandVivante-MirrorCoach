function resetSessionDraft() {
  ...
  sessionDraft = {
    patientId, protocolId: protocol?.id || '', date: $('sessionDate')?.value || today(), activeMinutes: protocol?.minutes || 30, painPre: getPatient(patientId)?.pain || 0, painPost: getPatient(patientId)?.pain || 0,
...
function firstIncompletePhase() {
  const index = PHASES.findIndex((_, i) => !completedPhases.has(i));
  return index === -1 ? PHASES.length - 1 : index;
}
function renderPhaseTabs() {
  const unlocked = firstIncompletePhase();
  $('phaseTabs').innerHTML = PHASES.map((p, i) => {
    const locked = i > unlocked;
    return `<button type="button" class="phase-tab ${i === phaseIndex ? 'active' : ''} ${completedPhases.has(i) ? 'done' : ''} ${locked ? 'locked' : ''}" data-phase="${i}" ${locked ? 'disabled aria-disabled="true"' : ''}>${p.letter} · ${p.name} · ${Math.round(p.seconds / 60)} min</button>`;
  }).join('');
}
...
function phaseValid(index) {
  ...
  if (index === 4 && N(sessionDraft.transfer) > N(sessionDraft.transferAttempted)) return 'Successful transfer attempts cannot exceed total attempts.';
  if (index === 4 && N(sessionDraft.activeMinutes) <= 0) return 'Record the actual delivered RMHT minutes before saving.';
...
function saveSession() {
  const session = {
    id: uid('session'), patientId: sessionDraft.patientId, protocolId: sessionDraft.protocolId, date: sessionDraft.date || today(), activeMinutes: N(sessionDraft.activeMinutes),
...
function goToPhase(index) {
  ...
  const unlocked = firstIncompletePhase();
  if (target > unlocked) return showStatus('Complete each preceding phase before moving ahead.', 'error');
  stopTimer(); phaseIndex = target; phaseSecondsLeft = PHASES[phaseIndex].seconds; renderSession();
}
