...
const APP_SCRIPTS = ['js/01-base.js', 'js/02-exercises.js', 'js/03-tutorial-content.js', 'js/04-manual-content.js', 'js/05-sample-state.js', 'js/06-core.js', 'js/07-patients-protocols.js', 'js/08-session.js', 'js/09-library-tutorial.js', 'js/10-manual-outcomes-research.js', 'js/11-init.js'];
async function loadMirrorCoach() {
  const root = document.getElementById('mirrorcoachRoot');
  try {
    const responses = await Promise.all(FRAGMENTS.map(path => fetch(path)));
    ...
    for (const src of APP_SCRIPTS) {
      await new Promise((resolve, reject) => {
        const script = document.createElement('script'); script.src = `${src}?v=20260713-3`; script.onload = resolve; script.onerror = reject; document.body.appendChild(script);
      });
    }
  } catch (error) {
